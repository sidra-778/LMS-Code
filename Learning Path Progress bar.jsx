...
// Added: calculates 0-100 progress for a path based on completed courses
function getPathProgress(path) {
  const total = path.courses.length;
  if (total === 0) return 0;
  const completed = path.courses.filter(
    (pc) => pc.completed || pc.course?.completed
  ).length;
  return Math.round((completed / total) * 100);
}

// Added: maps percent to a color band -> red, yellow, light green, green
function getProgressColor(percent) {
  if (percent < 25) return { bar: "bg-red-500", track: "bg-red-100", label: "Just Started" };
  if (percent < 50) return { bar: "bg-yellow-500", track: "bg-yellow-100", label: "In Progress" };
  if (percent < 75) return { bar: "bg-green-400", track: "bg-green-100", label: "Almost There" };
  return { bar: "bg-green-500", track: "bg-green-100", label: "Complete" };
}

// Added: the single progress bar component
function PathProgressBar({ path }) {
  const percent = getPathProgress(path);
  const { bar, track, label } = getProgressColor(percent);
          ...
          {paths.map(path => (
            <div key={path.id} className="bg-white rounded-2xl border shadow-sm overflow-hidden flex flex-col md:flex-row">
              <div className="p-8 md:w-1/3 bg-gray-50 border-r border-gray-100 flex flex-col justify-center">
                ...
                <div className="flex items-center gap-2 text-sm font-bold text-primary bg-green-50 px-3 py-1.5 rounded-full w-fit mt-4">
